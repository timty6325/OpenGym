...
declare player public.waitlist_players; conflict_player public.waitlist_players;
begin
  ...
  select * into player from public.waitlist_players where user_id=auth.uid() and status<>'left' for update;
  ...
  select * into conflict_player from public.waitlist_players
    where device_id=p_device_id and status<>'left' and id<>player.id for update;
  if conflict_player.id is not null then
    raise exception 'This browser already has a player on the waitlist.';
  end if;
  update public.waitlist_players set device_id=p_device_id,updated_at=now() where id=player.id;
...
returns jsonb language plpgsql security definer set search_path=public as $$
declare player public.waitlist_players; joined jsonb; joined_id uuid;
begin
  if auth.uid() is null then raise exception 'You must be signed in.'; end if;
  if p_device_id is null or length(p_device_id) < 16 or length(p_device_id) > 100 then
    raise exception 'Invalid device identifier.';
  ...
  perform pg_advisory_xact_lock(hashtext(p_device_id));
  select * into player from public.waitlist_players where device_id=p_device_id and status<>'left' for update;
  if player.id is not null then
    update public.waitlist_players set user_id=null where user_id=auth.uid() and id<>player.id;
    update public.waitlist_players set user_id=auth.uid(),updated_at=now() where id=player.id;
  ...
  update public.waitlist_players set device_id=p_device_id,updated_at=now() where id=joined_id;
  return joined;
end; $$;
