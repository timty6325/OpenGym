-- Session hosts can appoint other hosts in every waitlist mode. Their facility
-- is capped at two active hosts; facility admins are intentionally uncapped.
...
begin
  ...
  where facility_id=fid and user_id=auth.uid() and is_host
  ...
  perform pg_advisory_xact_lock(hashtext('waitlist-session-host-limit-'||fid::text));
  select * into target
  from public.waitlist_players
  ...
  if p_is_host and not caller_is_admin then
    select count(*) into host_count
    from public.waitlist_players
    where facility_id=fid and is_host
      and status in ('current','waiting','sitout','rejoin') and id<>target.id;
    if host_count>=2 then raise exception 'A session can have no more than two hosts.'; end if;
  end if;

  update public.waitlist_players
  set is_host=p_is_host,updated_at=now()
  where facility_id=fid and id=target.id;
  ...
  values(fid,auth.uid(),actor_label,case when p_is_host then 'host_appointed' else 'host_removed' end,
  ...
  values(fid,target.user_id,case when p_is_host then 'HOST_APPOINTED|'||actor_label||' appointed you as a Session Host.' else 'HOST_REMOVED|The admin removed your Session Host permissions.' end);
