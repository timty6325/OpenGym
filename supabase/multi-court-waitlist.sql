...
returns jsonb language plpgsql security definer set search_path=public as $$
declare player public.waitlist_players; source_court integer; moving_count integer; target_position bigint; open_spots integer; candidate record;
begin
  if not public.is_waitlist_operator() then raise exception 'Admin or host access required.'; end if;
  ...
  if p_status='waiting' then
    select queue_position into target_position from public.waitlist_players
      where status='waiting' and id<>player.id
    ...
    if target_position is null then select coalesce(max(queue_position),0)+1 into target_position
      from public.waitlist_players where status in('current','waiting','sitout'); end if;
    ...
    with moving as(select id,row_number()over(order by queue_position,id) rn
      from public.waitlist_players where id=player.id or(player.group_id is not null and group_id=player.group_id))
    ...
    if source_court is not null then
      ...
      for candidate in select coalesce(group_id,id) block_id,count(*)::integer block_size,min(queue_position) first_position
        from public.waitlist_players where status='waiting'
          and id<>player.id and(player.group_id is null or group_id is distinct from player.group_id)
    ...
    select queue_position into target_position from public.waitlist_players
      where status='current' and court_number=source_court and id<>player.id
        and(player.group_id is null or group_id is distinct from player.group_id)
      order by queue_position offset greatest(p_index,0) limit 1;
    if target_position is null then select coalesce(max(queue_position),0)+1 into target_position
      from public.waitlist_players where status='current' and court_number=source_court; end if;
    update public.waitlist_players set queue_position=queue_position*1000
      where status='current' and court_number=source_court;
    with moving as(select id,row_number()over(order by queue_position,id) rn
      from public.waitlist_players where id=player.id or(player.group_id is not null and group_id=player.group_id))
    update public.waitlist_players p set queue_position=target_position*1000-moving_count+moving.rn,
      updated_at=now() from moving where p.id=moving.id;
  else
    -- A waiting player dropped on a court takes the first available court spot.
    perform public.fill_open_court_slots();
  end if;
...
declare clean_first text:=public.clean_player_name(p_first_name); clean_last text:=public.clean_player_name(p_last_name);
  player public.waitlist_players; active_count integer; next_position bigint; shown_name text; fid uuid:=public.current_facility_id();
begin
  ...
  perform pg_advisory_xact_lock(7429101);
  select * into player from public.waitlist_players where facility_id=fid and user_id=auth.uid();
  ...
  if clean_last='' and exists(select 1 from public.waitlist_players where facility_id=fid and lower(first_name)=lower(clean_first) and status<>'left') then
  ...
  on conflict(facility_id,user_id) do update set first_name=excluded.first_name,last_name=excluded.last_name,
