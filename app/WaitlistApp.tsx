...
export default function App() {
  ...
  return <Shell>
    ...
    {hostTutorial&&<HostTutorial mode={config.mode} step={hostTutorialStep} next={()=>hostTutorialStep<getHostTutorialSteps(config.mode).length-1?setHostTutorialStep(value=>value+1):setHostTutorial(false)} back={()=>setHostTutorialStep(value=>Math.max(0,value-1))} skip={()=>setHostTutorial(false)}/>}
...
const teamHostTutorialSteps=[
 {title:'Next Game',message:'Advance the correct court here. For King of the Court, select the winning team; for 2 on, 2 off, both teams rotate while remaining intact.',target:'.next-game-tool,.court-next-button'},
