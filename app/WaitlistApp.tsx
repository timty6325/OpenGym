...
type Player = { id:string; user_id:string|null; first_name:string; last_name:string; display_name:string; status:PlayerStatus; queue_position:number|null; restricted:boolean; group_id:string|null; is_host:boolean; court_number:number|null; sitout_priority?:boolean; sitout_from_game?:number|null };
type Game = { id:string; game_number:number; court_number:number; player_names:string[]; ended_at:string };
type Config = { game_number:number; max_players:number; court_count:number; mode:'regular'|'rejoin'|'teams'; geofence_enabled:boolean; geofence_radius_m:number };
type Court = { court_number:number; game_number:number; started_at:string };
...
export default function App() {
  ...
  const savedQueuePositions=useMemo(()=>new Map(players.map(player=>[player.id,player.queue_position])),[players]);
  ...
  if(screen==='welcome')return <Shell><section className="auth-card"><Logo/><button className="hero-button" disabled={busy} onClick={()=>void startGuestFlow()}>{busy?'Starting guest modeâ€¦':'Continue as guest'}</button><button className="admin-link" onClick={()=>setScreen('admin')}>Admin</button><p className="fine">Join the live volleyball queue from your phone.</p></section>{notice&&<Modal notice={notice} close={()=>setNotice(null)} busy={busy}/>}</Shell>;
  ...
  if(me?.status==='rejoin'&&rejoinResponse){const remaining=Math.max(0,Math.ceil((new Date(rejoinResponse.expires_at).getTime()-returnClock)/1000));return <Shell><section className="auth-card rejoin-card"><Logo/><span className="kicker">REJOIN WAITLIST</span><h1>Do you want to rejoin?</h1><p>{pendingNextGameEvent?`${pendingNextGameEvent.message} Do you want to rejoin? Your position is saved.`:'Your position is saved.'} Choose before the timer reaches zero or youâ€™ll automatically leave the waitlist.</p><div className="return-countdown rejoin-countdown"><strong>{formatCountdown(remaining)}</strong><span>left to rejoin</span></div><div className="rejoin-actions"><button className="next" disabled={remaining===0} onClick={()=>{setPendingNextGameEvent(null);void answerRejoin('stay')}}>Rejoin</button><button className="danger" onClick={()=>{setPendingNextGameEvent(null);void answerRejoin('leave')}}>Leave</button></div></section>{notice&&<Modal notice={notice} close={()=>setNotice(null)} busy={busy}/>}</Shell>}
  if(screen==='name')return <Shell><section className="auth-card"><button className="back" onClick={()=>setScreen('welcome')}>â† Go back</button><h1>What should we call you?</h1><p>Your name is added to the waitlist as soon as you continue.</p><form onSubmit={join}><label>First name<input value={first} onChange={e=>setFirst(e.target.value)} placeholder="First name"/></label><label>Last initial or name<input value={last} onChange={e=>setLast(e.target.value)} placeholder="Optional: Last initial or name"/></label><button className="hero-button" disabled={busy}>Join waitlist</button></form></section>{notice&&<Modal notice={notice} close={()=>setNotice(null)} busy={busy}/>}</Shell>;

  return <Shell>
    <header className="topbar"><Logo compact/><div className="top-actions">{pushSupported()&&!notifications&&<button className="icon-button" onClick={turnOnNotifications}>Enable alerts</button>}<button className="icon-button" onClick={()=>ask('Log out?','This will remove you from the waitlist and sign you out.','Log out',async()=>{await rpc('leave_waitlist',{},false);await logout()})}>Log out</button><label className="language-picker" aria-label="Change language"><span className="language-symbol" aria-hidden="true"><i>ðŸŒ</i><b>{language==='en'?'ENG':language==='es'?'ESP':'ä¸­æ–‡'}</b></span><select value={language} onChange={event=>setLanguage(event.target.value as AppLanguage)}><option value="en">English</option><option value="es">EspaÃ±ol</option><option value="zh-CN">ç®€ä½“ä¸­æ–‡</option></select></label></div></header>
    <main className="queue-page">
      <section className="game-heading"><div><span className="kicker">LIVE QUEUE {admin?'Â· ADMIN':''}</span><h1>{translateUiText(courts.length>1?`${courts.length} Courts`:`Game ${courts[0]?.game_number??config.game_number}`,language)}</h1></div><span className="live-pill"><i/>Live</span></section>
...
function QueueCard({title,subtitle,status,players,start,savedQueuePositions,me,admin=false,operator=false,spotlight=false,groupSpotlight=false,editing,editName,setEditing,setEditName,saveName,requestGroup,leaveGroup,leaveOwnGroup,adminLeaveGroup,permissions,adminSitOut,adminLeave,dragging,dragOver,setDragging,setDragOver,movePlayer,projections,projectedCourts}:{title:string;subtitle:string;status:'current'|'waiting';players:Player[];start:number;savedQueuePositions:Map<string,number|null>;me?:Player;admin?:boolean;operator?:boolean;spotlight?:boolean;groupSpotlight?:boolean;editing:string|null;editName:string;setEditing:(v:string|null)=>void;setEditName:(v:string)=>void;saveName:(p:Player)=>void;requestGroup:(p:Player)=>Promise<void>;leaveGroup:(p:Player)=>Promise<void>;leaveOwnGroup:()=>void;adminLeaveGroup:(p:Player)=>Promise<void>;permissions:(p:Player)=>void;adminSitOut:(p:Player)=>void;adminLeave:(p:Player)=>void;dragging:string|null;dragOver:DropPlacement|null;setDragging:(v:string|null)=>void;setDragOver:(v:DropPlacement|null)=>void;movePlayer:(id:string,status:'current'|'waiting',index:number)=>Promise<void>;projections?:Map<string,number>;projectedCourts?:Map<string,number>}){
 ...
 useEffect(()=>{if(!projectedCourts)return;const added:HTMLElement[]=[];for(const [playerId,court] of projectedCourts){const row=cardRef.current?.querySelector<HTMLElement>(`[data-player-id="${playerId}"]`);const gameLabel=row?.querySelector<HTMLElement>('.player-name small:not(.sitout-priority-label)');if(!gameLabel||gameLabel.querySelector('.projected-court'))continue;const label=document.createElement('span');label.className='projected-court';label.textContent=` Â· Court ${court}`;gameLabel.appendChild(label);added.push(label)}return()=>added.forEach(label=>label.remove())},[projectedCourts,players]);
 ...
 const dragOverRef=useRef<DropPlacement|null>(dragOver);const acceptedTargetY=useRef<number|null>(null);
 ...
 const startAutoScroll=()=>{if(mobileDrag.current.frame!==null)return;const tick=()=>{if(!mobileDrag.current.active){mobileDrag.current.frame=null;return;}const y=mobileDrag.current.lastY;const edge=Math.min(110,window.innerHeight*.18);const bounds=queueDragBounds();const direction=y<edge&&Boolean(bounds&&bounds.top<0)?-1:y>window.innerHeight-edge&&Boolean(bounds&&bounds.bottom>window.innerHeight)?1:0;if(direction){const strength=Math.max(.25,1-Math.min(y,window.innerHeight-y)/edge);window.scrollBy(0,direction*(5+13*strength));positionPlayerDragPreview(mobileDrag.current.preview!,mobileDrag.current.lastX,y);updateMobileTarget(mobileDrag.current.lastX,y);}mobileDrag.current.frame=requestAnimationFrame(tick)};mobileDrag.current.frame=requestAnimationFrame(tick);};
 useEffect(()=>{if(!operator)return;const beginDesktopDrag=(event:PointerEvent)=>{if(document.body.classList.contains('admin-group-selecting')||event.pointerType!=='mouse'||event.button!==0)return;const target=event.target as HTMLElement;if(target.closest('button,input'))return;const row=target.closest<HTMLElement>('[data-player-id]');if(!row||row.dataset.playerStatus!==status)return;const player=players.find(item=>item.id===row.dataset.playerId);if(!player)return;event.preventDefault();const state=mobileDrag.current;state.startX=state.lastX=event.clientX;state.startY=state.lastY=event.clientY;state.player=player;state.active=true;setDragging(player.id);row.setPointerCapture(event.pointerId);const preview=createPlayerDragPreview(player,players,start);preview.classList.add('mobile-admin-drag-preview');document.body.appendChild(preview);state.preview=preview;document.body.classList.add('mobile-admin-dragging');positionPlayerDragPreview(preview,event.clientX,event.clientY);startAutoScroll();};document.addEventListener('pointerdown',beginDesktopDrag,{capture:true});return()=>document.removeEventListener('pointerdown',beginDesktopDrag,{capture:true});},[operator,players,start,status]);
