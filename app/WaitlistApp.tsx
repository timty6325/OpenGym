...
export default function App() {
  ...
  async function recordKingWinner(courtNumber:number,winnerId:string){
    setBusy(true);const {data,error}=await supabase.rpc('end_king_game',{p_court_number:courtNumber,p_winning_team_id:winnerId});setBusy(false);
  ...
  function askKingWinner(courtNumber:number){
    ...
    if(operator){
      setNotice({title:'Which team won?',message:'Select the team that won this game.',confirm:teamLabel(active[0]),actionTone:'success',action:()=>recordKingWinner(courtNumber,active[0].id),cancelLabel:teamLabel(active[1]),cancelTone:'success',cancelAction:()=>recordKingWinner(courtNumber,active[1].id),blocking:true});
      return;
    }
  ...
  return <Shell>
    ...
    <main className="queue-page">
      ...
      {config.mode==='teams'&&<KingBoard teams={kingTeams} courts={courts} me={me} admin={admin} host={host} busy={busy} maxWins={config.king_max_wins} changeMode={changeWaitlistMode} setMaxWins={value=>void rpc('set_king_max_wins',{p_max_wins:value},false)} addPlayer={()=>setScreen('add-player')} nextGame={confirmKingNext} joinTeam={joinKingTeam} adminSitOut={confirmAdminSitOut} adminLeave={confirmAdminLeave} permissions={setPermissionPlayer}/>}
...
function KingTeamCard({team,me,canJoin,busy,joinTeam,start,side,operator,admin,adminSitOut,adminLeave,permissions}:{team:KingTeam;me:Player|null;canJoin:boolean;busy:boolean;joinTeam:(id:string)=>Promise<void>;start:number;side:number;operator:boolean;admin:boolean;adminSitOut:(player:Player)=>void;adminLeave:(player:Player)=>void;permissions:(player:Player)=>void}){
 const own=team.id===me?.team_id;const slots=[...team.members,...Array.from({length:Math.max(0,6-team.members.length)},()=>null)];
 return <section className={`king-team-card queue-team-block ${own?'own-king-team':''}`}><div className="team-block-label"><strong>Team {side}</strong>{team.status==='current'&&<span>{team.consecutive_wins} win{team.consecutive_wins===1?'':'s'}</span>}</div>{slots.map((member,index)=><article className={`player-row king-player-row ${member?'':'king-empty-row'} ${member?.user_id===me?.user_id?'own':''}`} key={member?.id??`empty-${index}`}><span className="position">{start+index}</span><div className="player-name"><strong>{member?.display_name??'Open spot'}</strong></div>{member&&member.user_id===me?.user_id&&<span className="you">You</span>}{member&&operator&&<div className="admin-player-actions"><button className="admin-sitout-button" onClick={()=>adminSitOut(member)}>Sit out</button><button className="admin-leave-button" onClick={()=>adminLeave(member)}>Remove</button>{admin&&<button className="restrict-button" onClick={()=>permissions(member)}>Permissions</button>}</div>}</article>)}{team.status==='waiting'&&!own&&<button className="king-join-button" disabled={!canJoin||busy} onClick={()=>void joinTeam(team.id)}>{team.members.length>=6?'Team full':'Join +'}</button>}</section>
}
function KingBoard({teams,courts,me,admin,host,busy,maxWins,changeMode,setMaxWins,addPlayer,nextGame,joinTeam,adminSitOut,adminLeave,permissions}:{teams:KingTeam[];courts:Court[];me:Player|null;admin:boolean;host:boolean;busy:boolean;maxWins:number|null;changeMode:(mode:Config['mode'])=>Promise<void>;setMaxWins:(value:number|null)=>void;addPlayer:()=>void;nextGame:(court:number)=>void;joinTeam:(id:string)=>Promise<void>;adminSitOut:(player:Player)=>void;adminLeave:(player:Player)=>void;permissions:(player:Player)=>void}){
 const operator=admin||host;const waiting=teams.filter(team=>team.status==='waiting').sort((a,b)=>a.queue_position-b.queue_position);
 return <div className="king-board"><section className="king-mode-heading"><div><span className="kicker">TEAMS MODE</span><h2>King of the Court</h2><p>Winners stay. Waiting teams rotate through one shared line.</p></div>{operator&&<label className="king-win-limit">Win limit<select value={maxWins??'unlimited'} onChange={event=>setMaxWins(event.target.value==='unlimited'?null:Number(event.target.value))}><option value="1">1 win</option><option value="2">2 wins</option><option value="3">3 wins</option><option value="unlimited">Unlimited</option></select></label>}</section>{courts.map(court=>{const active=teams.filter(team=>team.status==='current'&&team.court_number===court.court_number).sort((a,b)=>(a.court_side??1)-(b.court_side??1));const ownCourt=active.some(team=>team.id===me?.team_id);return <section className="king-court queue-card" key={court.court_number}><header><h2>{courts.length>1?`COURT ${court.court_number} - Game ${court.game_number}`:`Game ${court.game_number}`}</h2><span>{active.reduce((total,team)=>total+team.members.length,0)} playing</span></header>{courts.length>1&&(operator||ownCourt)&&<button className="court-next-button" disabled={busy||active.length!==2||Boolean(me?.restricted)} onClick={()=>nextGame(court.court_number)}>Next game (Court {court.court_number})</button>}<div className="king-current-teams">{active.map((team,index)=><KingTeamCard key={team.id} team={team} me={me} canJoin={false} busy={busy} joinTeam={joinTeam} start={index*6+1} side={index+1} operator={operator} admin={admin} adminSitOut={adminSitOut} adminLeave={adminLeave} permissions={permissions}/>)}{active.length<2&&Array.from({length:2-active.length},(_,index)=><section className="king-team-card queue-team-block empty-team-block" key={`empty-team-${index}`}><div className="team-block-label"><strong>Team {active.length+index+1}</strong></div>{Array.from({length:6},(__,slot)=><article className="player-row king-player-row king-empty-row" key={slot}><span className="position">{(active.length+index)*6+slot+1}</span><div className="player-name"><strong>Open spot</strong></div></article>)}</section>)}</div></section>})}<section className="king-waitlist queue-card"><header><h2>Waitlist</h2><span>{waiting.length?`${waiting.length} team${waiting.length===1?'':'s'} waiting`:'No one waiting'}</span></header>{waiting.length?<div className="king-waiting-list">{waiting.map((team,index)=><KingTeamCard key={team.id} team={team} me={me} canJoin={team.members.length<6} busy={busy} joinTeam={joinTeam} start={13+index*6} side={index+1} operator={operator} admin={admin} adminSitOut={adminSitOut} adminLeave={adminLeave} permissions={permissions}/>)}</div>:<p className="empty">Players will appear here.</p>}</section></div>
}
