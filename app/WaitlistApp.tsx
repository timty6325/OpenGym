...
export default function App() {
  ...
  const geofenceRemovalInProgress=useRef(false); const expiredRejoinHandled=useRef(false); const lastResumeRefresh=useRef(0); const adminMoveInProgress=useRef(false); const handledNotificationIds=useRef(new Set<string>()); const ownHostStatus=useRef(false); const hostTransitionHandledAt=useRef(0);
  ...
  async function boot(){
    ...
    const channel=supabase.channel('live-waitlist')
      .on('postgres_changes',{event:'*',schema:'public',table:'waitlist_players'},payload=>{
        const changed=payload.new as Player;const isOwnPlayer=changed.user_id===session.user.id;
        if(isOwnPlayer){const wasHost=ownHostStatus.current;const isHost=Boolean(changed.is_host);ownHostStatus.current=isHost;
          if(!wasHost&&isHost){hostTransitionHandledAt.current=Date.now();setPlayers(items=>items.map(player=>player.user_id===session.user.id?{...player,is_host:true}:player));setHostAppointmentNotice('The admin appointed you as a Session Host.');window.setTimeout(()=>void clearUnreadHostNotifications(session.user.id,'HOST_APPOINTED|'),250);}
          if(wasHost&&!isHost){setHostAppointmentNotice(null);setHostTutorial(false);setNotice({title:'Host permissions removed',message:'The admin removed your Session Host permissions.'});window.setTimeout(()=>void clearUnreadHostNotifications(session.user.id,'HOST_REMOVED|'),250);}
        }
        if(!adminMoveInProgress.current)void refresh();
      })
  ...
  async function refresh(activeUser?:User|null){
    ...
    const activeUid=(activeUser??user)?.id;const activeHost=Boolean(playerRows.find(item=>item.user_id===activeUid)?.is_host);ownHostStatus.current=activeHost;
