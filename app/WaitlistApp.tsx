...
function resolveDropPlacement(x:number,y:number,fallback:DropPlacement|null=null):DropPlacement|null{
 ...
 if(fallback&&fallback.status===status&&Math.abs(point.y-midpoint)<8)return fallback;
...
function QueueCard({title,subtitle,status,players,start,savedQueuePositions,me,admin=false,operator=false,spotlight=false,groupSpotlight=false,editing,editName,setEditing,setEditName,saveName,requestGroup,leaveGroup,leaveOwnGroup,adminLeaveGroup,permissions,adminSitOut,adminLeave,dragging,dragOver,setDragging,setDragOver,movePlayer,projections}:{title:string;subtitle:string;status:'current'|'waiting';players:Player[];start:number;savedQueuePositions:Map<string,number|null>;me?:Player;admin?:boolean;operator?:boolean;spotlight?:boolean;groupSpotlight?:boolean;editing:string|null;editName:string;setEditing:(v:string|null)=>void;setEditName:(v:string)=>void;saveName:(p:Player)=>void;requestGroup:(p:Player)=>Promise<void>;leaveGroup:(p:Player)=>Promise<void>;leaveOwnGroup:()=>void;adminLeaveGroup:(p:Player)=>Promise<void>;permissions:(p:Player)=>void;adminSitOut:(p:Player)=>void;adminLeave:(p:Player)=>void;dragging:string|null;dragOver:DropPlacement|null;setDragging:(v:string|null)=>void;setDragOver:(v:DropPlacement|null)=>void;movePlayer:(id:string,status:'current'|'waiting',index:number)=>Promise<void>;projections?:Map<string,number>}){
 ...
 const updateMobileTarget=(x:number,y:number)=>{const point=constrainQueueDragPoint(x,y);setDragOver(resolveDropPlacement(point.x,point.y,dragOver));};
 ...
 const finishMobileDrag=(x:number,y:number)=>{const state=mobileDrag.current;if(state.timer!==null)window.clearTimeout(state.timer);document.removeEventListener('touchmove',preventNativeTouchScroll);stopAutoScroll();if(state.active&&state.player){const placement=resolveDropPlacement(x,y,dragOver);if(placement)void movePlayer(state.player.id,placement.status,placement.index);}state.preview?.remove();document.body.classList.remove('mobile-admin-dragging');mobileDrag.current={timer:null,active:false,preview:null,player:null,startX:0,startY:0,lastX:0,lastY:0,frame:null};setDragging(null);setDragOver(null);};
