...
function resolveDropPlacement(x:number,y:number,fallback:DropPlacement|null=null):DropPlacement|null{
 ...
 const hovered=document.elementsFromPoint(point.x,point.y).map(element=>(element as HTMLElement).closest<HTMLElement>('[data-player-id]')).find(row=>row&&card.contains(row)&&!row.classList.contains('dragging'))??null;
 let chosen=hovered?blocks.find(block=>block.first===hovered||block.last===hovered||Boolean(hovered.dataset.groupId&&hovered.dataset.groupId===block.first.dataset.groupId)):null;
 if(!chosen){chosen=blocks.reduce((best,block)=>{const rect=block.first.getBoundingClientRect();const lastRect=block.last.getBoundingClientRect();const distance=Math.abs(point.y-(rect.top+lastRect.bottom)/2);if(!best)return{block,distance};return distance<best.distance?{block,distance}:best},null as {block:{first:HTMLElement;last:HTMLElement};distance:number}|null)?.block??blocks[0];}
 const firstIndex=rows.indexOf(chosen.first);const lastIndex=rows.indexOf(chosen.last);const rectFirst=chosen.first.getBoundingClientRect();const rectLast=chosen.last.getBoundingClientRect();const after=point.y>=(rectFirst.top+rectLast.bottom)/2;
 return after?{status,index:lastIndex+1,marker:`after:${chosen.last.dataset.playerId}`,courtNumber}:{status,index:firstIndex,marker:`before:${chosen.first.dataset.playerId}`,courtNumber};
}
...
function QueueCard({title,subtitle,status,players,start,savedQueuePositions,me,admin=false,operator=false,spotlight=false,groupSpotlight=false,editing,editName,setEditing,setEditName,saveName,requestGroup,leaveGroup,leaveOwnGroup,adminLeaveGroup,permissions,adminSitOut,adminLeave,dragging,dragOver,setDragging,setDragOver,movePlayer,projections,projectedCourts}:{title:string;subtitle:string;status:'current'|'waiting';players:Player[];start:number;savedQueuePositions:Map<string,number|null>;me?:Player;admin?:boolean;operator?:boolean;spotlight?:boolean;groupSpotlight?:boolean;editing:string|null;editName:string;setEditing:(v:string|null)=>void;setEditName:(v:string)=>void;saveName:(p:Player)=>void;requestGroup:(p:Player)=>Promise<void>;leaveGroup:(p:Player)=>Promise<void>;leaveOwnGroup:()=>void;adminLeaveGroup:(p:Player)=>Promise<void>;permissions:(p:Player)=>void;adminSitOut:(p:Player)=>void;adminLeave:(p:Player)=>void;dragging:string|null;dragOver:DropPlacement|null;setDragging:(v:string|null)=>void;setDragOver:(v:DropPlacement|null)=>void;movePlayer:(id:string,status:'current'|'waiting',index:number,courtNumber?:number|null)=>Promise<void>;projections?:Map<string,number>;projectedCourts?:Map<string,number>}){
 ...
 const finishMobileDrag=(x:number,y:number)=>{const state=mobileDrag.current;if(state.timer!==null)window.clearTimeout(state.timer);document.removeEventListener('touchmove',preventNativeTouchScroll);stopAutoScroll();const placement=state.active&&state.player?resolveDropPlacement(x,y,dragOverRef.current):null;const playerId=state.player?.id??null;state.preview?.remove();clearDragArtifacts();mobileDrag.current={timer:null,active:false,preview:null,player:null,startX:0,startY:0,lastX:0,lastY:0,frame:null};setDragging(null);resetDragTarget();if(playerId&&placement)void movePlayer(playerId,placement.status,placement.index,placement.courtNumber);};
