...
function KingBoard({teams,courts,me,admin,host,busy,nextGame,joinTeam,joinEmptyTeam,setCourtRules,movePlayer,adminSitOut,adminLeave,permissions}:{teams:KingTeam[];courts:Court[];me:Player|null;admin:boolean;host:boolean;busy:boolean;nextGame:(court:number)=>void;joinTeam:(team:KingTeam,side:number)=>void;joinEmptyTeam:(side:number)=>void;setCourtRules:(court:number,mode:'rotation'|'king',maxWins:number|null)=>Promise<void>;movePlayer:(playerId:string,targetTeamId:string,index:number)=>Promise<void>;adminSitOut:(player:Player)=>void;adminLeave:(player:Player)=>void;permissions:(player:Player)=>void}){
 ...
 const dragRef=useRef<{player:Player|null;preview:HTMLElement|null;timer:number|null;active:boolean;lastX:number;lastY:number;startX:number;startY:number;frame:number|null}>({player:null,preview:null,timer:null,active:false,lastX:0,lastY:0,startX:0,startY:0,frame:null});
 ...
 const activateDrag=(player:Player,event:ReactPointerEvent<HTMLElement>,start:number)=>{const row=(event.target as HTMLElement).closest<HTMLElement>('[data-player-id]');if(!row)return;captureDragPickupViewport(row);const state=dragRef.current;state.player=player;state.timer=null;state.active=true;const preview=createPlayerDragPreview(player,[player],start);preview.classList.add('mobile-admin-drag-preview','king-player-drag-preview');document.body.appendChild(preview);state.preview=preview;document.body.classList.add('mobile-admin-dragging');positionPlayerDragPreview(preview,state.lastX,state.lastY);setDragging(player.id);navigator.vibrate?.(35);resolveTarget(state.lastX,state.lastY);startAutoScroll()};
 const beginDrag=(player:Player,event:ReactPointerEvent<HTMLElement>,start:number)=>{if(busy||(event.target as HTMLElement).closest('button,input'))return;event.preventDefault();const state=dragRef.current;state.player=player;state.lastX=event.clientX;state.lastY=event.clientY;state.startX=event.clientX;state.startY=event.clientY;if(event.pointerType==='mouse')activateDrag(player,event,start);else state.timer=window.setTimeout(()=>activateDrag(player,event,start),280)};
 useEffect(()=>{if(!operator)return;const move=(event:PointerEvent)=>{const state=dragRef.current;state.lastX=event.clientX;state.lastY=event.clientY;if(!state.active){if(state.timer!==null&&Math.hypot(event.clientX-state.startX,event.clientY-state.startY)>18){clearTimeout(state.timer);state.timer=null;state.player=null;}return;}event.preventDefault();if(state.preview)positionPlayerDragPreview(state.preview,event.clientX,event.clientY);resolveTarget(event.clientX,event.clientY)};const finish=()=>{const state=dragRef.current;if(state.timer!==null)clearTimeout(state.timer);stopAutoScroll();const playerId=state.player?.id??null;const target=dropRef.current;state.preview?.remove();document.body.classList.remove('mobile-admin-dragging');dragRef.current={player:null,preview:null,timer:null,active:false,lastX:0,lastY:0,startX:0,startY:0,frame:null};dropRef.current=null;setDragging(null);setDrop(null);if(playerId&&target)void movePlayer(playerId,target.teamId,target.index)};document.addEventListener('pointermove',move,{passive:false});document.addEventListener('pointerup',finish);document.addEventListener('pointercancel',finish);return()=>{document.removeEventListener('pointermove',move);document.removeEventListener('pointerup',finish);document.removeEventListener('pointercancel',finish);stopAutoScroll()}},[operator,teams,busy]);
