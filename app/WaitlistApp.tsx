...
export default function App() {
  ...
  return <Shell>
    ...
    <main className="queue-page">
      ...
      {config.mode==='teams'&&<KingBoard teams={kingTeams} courts={courts} me={me} admin={admin} host={host} busy={busy} maxWins={config.king_max_wins} changeMode={changeWaitlistMode} setMaxWins={value=>void rpc('set_king_max_wins',{p_max_wins:value},false)} addPlayer={()=>setScreen('add-player')} nextGame={confirmKingNext} joinTeam={joinKingTeam}/>}
...
function KingTeamCard({team,me,canJoin,busy,joinTeam}:{team:KingTeam;me:Player|null;canJoin:boolean;busy:boolean;joinTeam:(id:string)=>Promise<void>}){
 const own=team.id===me?.team_id;const emptySlots=Math.max(0,6-team.members.length);
 return <article className={`king-team-card ${own?'own-king-team':''}`}><header><div><span>{team.name}</span><h3>{teamLabel(team)}</h3></div>{team.status==='current'&&<strong>{team.consecutive_wins} win{team.consecutive_wins===1?'':'s'}</strong>}</header><ol>{team.members.map(member=><li key={member.id}>{member.display_name}{member.user_id===me?.user_id&&<small>You</small>}</li>)}{Array.from({length:emptySlots},(_,index)=><li className="king-empty-slot" key={`empty-${index}`}>Open spot</li>)}</ol>{team.status==='waiting'&&!own&&<button className="king-join-button" disabled={!canJoin||busy} onClick={()=>void joinTeam(team.id)}>{team.members.length>=6?'Team full':'Join +'}</button>}</article>
}
function KingBoard({teams,courts,me,admin,host,busy,maxWins,changeMode,setMaxWins,addPlayer,nextGame,joinTeam}:{teams:KingTeam[];courts:Court[];me:Player|null;admin:boolean;host:boolean;busy:boolean;maxWins:number|null;changeMode:(mode:Config['mode'])=>Promise<void>;setMaxWins:(value:number|null)=>void;addPlayer:()=>void;nextGame:(court:number)=>void;joinTeam:(id:string)=>Promise<void>}){
 const operator=admin||host;const waiting=teams.filter(team=>team.status==='waiting').sort((a,b)=>a.queue_position-b.queue_position);
 return <div className="king-board"><section className="king-mode-heading"><div><span className="kicker">TEAMS MODE</span><h2>King of the Court</h2><p>Winners stay. Waiting teams rotate through one shared line.</p></div>{operator&&<div className="king-settings">{admin&&<select value="teams" onChange={event=>void changeMode(event.target.value as Config['mode'])}><option value="regular">Regular waitlist</option><option value="rejoin">Rejoin waitlist</option><option value="teams">Teams mode</option></select>}<label>Win limit<select value={maxWins??'unlimited'} onChange={event=>setMaxWins(event.target.value==='unlimited'?null:Number(event.target.value))}><option value="1">1 win</option><option value="2">2 wins</option><option value="3">3 wins</option><option value="unlimited">Unlimited</option></select></label><button className="add-player-tool" onClick={addPlayer}>Add player</button></div>}</section>{courts.map(court=>{const active=teams.filter(team=>team.status==='current'&&team.court_number===court.court_number).sort((a,b)=>(a.court_side??1)-(b.court_side??1));const ownCourt=active.some(team=>team.id===me?.team_id);return <section className="king-court" key={court.court_number}><header><div><span>Court {court.court_number}</span><h2>Game {court.game_number}</h2></div>{(operator||ownCourt)&&<button className="next" disabled={busy||active.length!==2||Boolean(me?.restricted)} onClick={()=>nextGame(court.court_number)}>Next game{courts.length>1?` (Court ${court.court_number})`:''}</button>}</header><div className="king-matchup">{active.map((team,index)=><div className="king-side" key={team.id}><span>Team {index+1}</span><KingTeamCard team={team} me={me} canJoin={false} busy={busy} joinTeam={joinTeam}/></div>)}{active.length<2&&<div className="king-awaiting">Waiting for {2-active.length} more team{active.length===1?'':'s'}</div>}</div></section>})}<section className="king-waitlist"><header><div><span className="kicker">SHARED TEAM WAITLIST</span><h2>Up next</h2></div><strong>{waiting.length} team{waiting.length===1?'':'s'}</strong></header>{waiting.length?<div className="king-team-grid">{waiting.map((team,index)=><div className="king-waiting-entry" key={team.id}><span className="king-position">{index+1}</span><KingTeamCard team={team} me={me} canJoin={team.members.length<6} busy={busy} joinTeam={joinTeam}/></div>)}</div>:<p className="empty">Teams with players will appear here.</p>}</section></div>
}
