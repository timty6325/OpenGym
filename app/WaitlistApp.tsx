...
type Notice = { title:string; message:string; confirm?:string; action?:()=>Promise<void>; onClose?:()=>void; actionTone?:'danger'|'success'; cancelTone?:'neutral'|'danger' } | null;
...
export default function App() {
  ...
  if(screen==='welcome')return <Shell><section className="auth-card"><Logo/><button className="hero-button" onClick={()=>setScreen('name')}>Continue as guest</button><button className="admin-link" onClick={()=>setScreen('admin')}>Admin</button><p className="fine">Join the live volleyball queue from your phone.</p></section>{notice&&<Modal notice={notice} close={()=>setNotice(null)} busy={busy}/>}</Shell>;
  ...
  return <Shell>
    ...
    <main className="queue-page">
      ...
      {admin&&<button className={`admin-group-launch ${adminGrouping?'active':''}`} onClick={adminGrouping?cancelAdminGrouping:startAdminGrouping}>{adminGrouping?'Cancel Grouping':'Group Up'}</button>}
      ...
      {adminGrouping&&<aside className="admin-group-toolbar"><span>{adminGroupIds.length}/6 selected</span><button className="group-done" onClick={previewAdminGroup}>Done</button><button className="group-cancel" onClick={cancelAdminGrouping}>Cancel</button></aside>}
      {me&&me.status!=='left'&&<section className={`my-actions ${onboarding==='tutorial'&&tutorialStep===0&&me.status!=='current'?'tutorial-focus':''}`}><span>Your actions</span><div>{me.status==='current'&&<button className={`next ${onboarding==='tutorial'&&tutorialStep===0?'tutorial-focus':''}`} disabled={busy||me.restricted} onClick={()=>ask('Start the next game?',`This will notify all players that ${me.display_name} advanced the queue. This cannot be quietly undone.`,'Next game',advanceGame)}>Next game</button>}<button className={`neutral ${onboarding==='tutorial'&&tutorialStep===1?'tutorial-focus':''}`} disabled={busy} onClick={()=>ask('Sit out one game?',"You’ll skip one game, then receive priority for the following game.",'Sit out',async()=>{await rpc('sit_out_one_game')})}>Sit out</button><button className={`danger ${onboarding==='tutorial'&&tutorialStep===2?'tutorial-focus':''}`} disabled={busy} onClick={()=>ask('Leave the waitlist?','This removes you from the current game or queue. You can join again later.','Leave',async()=>{await rpc('leave_waitlist',{},false)})}>Leave</button></div></section>}
      ...
      {!admin&&<button className="history-button your-history-button" onClick={()=>void openPlayerHistory()}>Your history <span>→</span></button>}
...
function TutorialCoach({step,next,back,skip}:{step:number;next:()=>void;back:()=>void;skip:()=>void}){const item=tutorialSteps[step];useEffect(()=>{const frame=requestAnimationFrame(()=>document.querySelector('.tutorial-focus')?.scrollIntoView({behavior:'smooth',block:'center'}));return()=>cancelAnimationFrame(frame)},[step]);return <><div className="tutorial-scrim"/><section className={`tutorial-coach ${step>=4?'tutorial-coach-contextual':''}`} role="dialog" aria-modal="true" aria-live="polite"><div className="tutorial-progress"><span>QUICK TOUR · {step+1} OF {tutorialSteps.length}</span><button onClick={skip}>Skip tutorial</button></div><h2>{item.title}</h2><p>{item.message}</p><div className="tutorial-actions">{step>0&&<button className="neutral" onClick={back}>Back</button>}<button className="next" onClick={next}>{step===tutorialSteps.length-1?'Finish':'Next'}</button></div></section></>}
...
function QueueCard({title,subtitle,status,players,start,me,admin=false,spotlight=false,editing,editName,setEditing,setEditName,saveName,requestGroup,leaveGroup,restrict,adminSitOut,adminLeave,dragging,dragOver,setDragging,setDragOver,movePlayer,projections}:{title:string;subtitle:string;status:'current'|'waiting';players:Player[];start:number;me?:Player;admin?:boolean;spotlight?:boolean;editing:string|null;editName:string;setEditing:(v:string|null)=>void;setEditName:(v:string)=>void;saveName:(p:Player)=>void;requestGroup:(p:Player)=>Promise<void>;leaveGroup:(p:Player)=>Promise<void>;restrict:(p:Player)=>void|Promise<unknown>;adminSitOut:(p:Player)=>void;adminLeave:(p:Player)=>void;dragging:string|null;dragOver:string|null;setDragging:(v:string|null)=>void;setDragOver:(v:string|null)=>void;movePlayer:(id:string,status:'current'|'waiting',index:number)=>Promise<void>;projections?:Map<string,number>}){
 ...
 const updateMobileTarget=(x:number,y:number)=>{const target=document.elementFromPoint(x,y)?.closest<HTMLElement>('[data-player-id]');setDragOver(target?.dataset.playerId??null);};
