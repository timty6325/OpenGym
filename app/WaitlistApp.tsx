...
function KingTeamCard({team,me,canJoin,busy,joinTeam,start,side}:{team:KingTeam;me:Player|null;canJoin:boolean;busy:boolean;joinTeam:(id:string)=>Promise<void>;start:number;side:number}){
 const own=team.id===me?.team_id;const slots=[...team.members,...Array.from({length:Math.max(0,6-team.members.length)},()=>null)];
 return <section className={`king-team-card queue-team-block ${own?'own-king-team':''}`}><div className="team-block-label"><strong>Team {side}</strong>{team.status==='current'&&<span>{team.consecutive_wins} win{team.consecutive_wins===1?'':'s'}</span>}</div>{slots.map((member,index)=><article className={`player-row king-player-row ${member?'':'king-empty-row'}`} key={member?.id??`empty-${index}`}><span className="position">{start+index}</span><div className="player-name"><strong>{member?.display_name??'Open spot'}</strong></div>{member?.user_id===me?.user_id&&<span className="you">You</span>}</article>)}{team.status==='waiting'&&!own&&<button className="king-join-button" disabled={!canJoin||busy} onClick={()=>void joinTeam(team.id)}>{team.members.length>=6?'Team full':'Join +'}</button>}</section>
}
