...
type Player = { id:string; user_id:string|null; first_name:string; last_name:string; display_name:string; status:PlayerStatus; queue_position:number|null; restricted:boolean; group_id:string|null; is_host:boolean; sitout_priority?:boolean; sitout_from_game?:number|null };
...
export default function App() {
  ...
  const tutorialWaiting=tutorialNeedsDemo?[...waiting,{id:'tutorial-demo-player',user_id:null,first_name:'Demo',last_name:'Player',display_name:'Demo Player',status:'waiting' as PlayerStatus,queue_position:(waiting.at(-1)?.queue_position??current.length)+1,restricted:false,group_id:null,is_host:false,sitout_priority:false,sitout_from_game:null}]:waiting;
  ...
  return <Shell>
    ...
    <main className="queue-page">
      <section className="game-heading"><div><span className="kicker">LIVE QUEUE {admin?'· ADMIN':''}</span><h1>{translateUiText(`Game ${config.game_number}`,language)}</h1></div><span className="live-pill"><i/>Live</span></section>
