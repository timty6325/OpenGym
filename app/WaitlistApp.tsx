...
function KingTeamCard({team,me,canJoin,busy,joinTeam,start,side,showStreak,operator,admin,adminSitOut,adminLeave,permissions,dragging,drop,beginDrag}:{team:KingTeam;me:Player|null;canJoin:boolean;busy:boolean;joinTeam:(team:KingTeam,side:number)=>void;start:number;side:number;showStreak:boolean;operator:boolean;admin:boolean;adminSitOut:(player:Player)=>void;adminLeave:(player:Player)=>void;permissions:(player:Player)=>void;dragging:string|null;drop:KingDropPlacement|null;beginDrag:(player:Player,event:ReactPointerEvent<HTMLElement>,start:number)=>void}){
 ...
 useLayoutEffect(()=>{const rows=[...document.querySelectorAll<HTMLElement>(`[data-king-team-id="${team.id}"] [data-player-id]`)];const next=new Map<string,DOMRect>();for(const row of rows){const id=row.dataset.playerId;if(!id)continue;const rect=row.getBoundingClientRect();next.set(id,rect);const before=previousRects.current.get(id);const delta=before?before.top-rect.top:0;row.getAnimations().forEach(animation=>animation.cancel());if(dragging&&Math.abs(delta)>1&&!matchMedia('(prefers-reduced-motion: reduce)').matches)row.animate([{transform:`translateY(${delta}px)`},{transform:'translateY(0)'}],{duration:150,easing:'cubic-bezier(.2,.8,.2,1)'});}previousRects.current=next;},[items.map(item=>item==='drop'?'drop':item.id).join('|'),dragging]);
