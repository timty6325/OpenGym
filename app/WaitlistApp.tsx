...
function KingTeamCard({team,me,canJoin,busy,joinTeam,start,side,showStreak,operator,admin,adminSitOut,adminLeave,permissions,dragging,drop,beginDrag}:{team:KingTeam;me:Player|null;canJoin:boolean;busy:boolean;joinTeam:(team:KingTeam,side:number)=>void;start:number;side:number;showStreak:boolean;operator:boolean;admin:boolean;adminSitOut:(player:Player)=>void;adminLeave:(player:Player)=>void;permissions:(player:Player)=>void;dragging:string|null;drop:KingDropPlacement|null;beginDrag:(player:Player,event:ReactPointerEvent<HTMLElement>,start:number)=>void}){
...
function KingBoard({teams,courts,me,admin,host,busy,nextGame,joinTeam,joinEmptyTeam,setCourtRules,movePlayer,adminSitOut,adminLeave,permissions}:{teams:KingTeam[];courts:Court[];me:Player|null;admin:boolean;host:boolean;busy:boolean;nextGame:(court:number)=>void;joinTeam:(team:KingTeam,side:number)=>void;joinEmptyTeam:(side:number)=>void;setCourtRules:(court:number,mode:'rotation'|'king',maxWins:number|null)=>Promise<void>;movePlayer:(playerId:string,targetTeamId:string,index:number)=>Promise<void>;adminSitOut:(player:Player)=>void;adminLeave:(player:Player)=>void;permissions:(player:Player)=>void}){
 ...
 const dragRef=useRef<{player:Player|null;row:HTMLElement|null;pointerId:number|null;preview:HTMLElement|null;timer:number|null;active:boolean;lastX:number;lastY:number;startX:number;startY:number;frame:number|null}>({player:null,row:null,pointerId:null,preview:null,timer:null,active:false,lastX:0,lastY:0,startX:0,startY:0,frame:null});
 ...
 const renderedDropPlacement=()=>{const placeholder=document.querySelector<HTMLElement>('.king-drop-placeholder');const card=placeholder?.closest<HTMLElement>('[data-king-team-id],[data-empty-king-target]');const teamId=card?candidateKey(card):null;if(placeholder&&card&&teamId){const rows=[...card.querySelectorAll<HTMLElement>('.king-player-row')];const index=rows.indexOf(placeholder);if(index>=0)return{teamId,index}}const emptyCard=document.querySelector<HTMLElement>('.king-empty-drop-target[data-empty-king-target]');const emptyTeamId=emptyCard?candidateKey(emptyCard):null;return emptyTeamId?{teamId:emptyTeamId,index:0}:null};
 ...
 const beginDrag=(player:Player,event:ReactPointerEvent<HTMLElement>,start:number)=>{if(busy||(event.target as HTMLElement).closest('button,input'))return;event.preventDefault();window.getSelection()?.removeAllRanges();document.body.classList.add('king-drag-holding');const row=(event.target as HTMLElement).closest<HTMLElement>('[data-player-id]');if(!row)return;const state=dragRef.current;state.player=player;state.row=row;state.pointerId=event.pointerId;state.lastX=event.clientX;state.lastY=event.clientY;state.startX=event.clientX;state.startY=event.clientY;if(event.pointerType==='mouse')activateDrag(player,start);else{document.addEventListener('touchmove',preventNativeTouchScroll,{passive:false});state.timer=window.setTimeout(()=>activateDrag(player,start),180)}};
 useEffect(()=>{if(!operator)return;const preventSelection=()=>{if(document.body.classList.contains('king-drag-holding'))window.getSelection()?.removeAllRanges()};const move=(event:PointerEvent)=>{const state=dragRef.current;state.lastX=event.clientX;state.lastY=event.clientY;if(!state.active){if(state.timer!==null&&Math.hypot(event.clientX-state.startX,event.clientY-state.startY)>24){clearTimeout(state.timer);state.timer=null;state.player=null;state.row=null;state.pointerId=null;document.body.classList.remove('king-drag-holding');document.removeEventListener('touchmove',preventNativeTouchScroll);}return;}event.preventDefault();if(state.preview)positionPlayerDragPreview(state.preview,event.clientX,event.clientY);resolveTarget(event.clientX,event.clientY);startAutoScroll()};const finish=()=>{const state=dragRef.current;if(state.timer!==null)clearTimeout(state.timer);document.removeEventListener('touchmove',preventNativeTouchScroll);stopAutoScroll();const playerId=state.active?state.player?.id??null:null;const target=state.active?renderedDropPlacement():null;state.preview?.remove();document.body.classList.remove('mobile-admin-dragging','king-drag-holding');dragRef.current={player:null,row:null,pointerId:null,preview:null,timer:null,active:false,lastX:0,lastY:0,startX:0,startY:0,frame:null};dropRef.current=null;setDragging(null);setDrop(null);clearEmptyTarget();if(playerId&&target)void movePlayer(playerId,target.teamId,target.index)};document.addEventListener('selectionchange',preventSelection);document.addEventListener('pointermove',move,{passive:false,capture:true});document.addEventListener('pointerup',finish,{capture:true});document.addEventListener('pointercancel',finish,{capture:true});return()=>{document.removeEventListener('selectionchange',preventSelection);document.removeEventListener('pointermove',move,{capture:true});document.removeEventListener('pointerup',finish,{capture:true});document.removeEventListener('pointercancel',finish,{capture:true});document.removeEventListener('touchmove',preventNativeTouchScroll);document.body.classList.remove('king-drag-holding');stopAutoScroll()}},[operator,teams,busy]);
 const card=(team:KingTeam,start:number,side:number,showStreak:boolean)=><KingTeamCard key={team.id} team={team} me={me} canJoin={!operator&&team.id!==me?.team_id&&team.members.length<6} busy={busy} joinTeam={joinTeam} start={start} side={side} showStreak={showStreak} operator={operator} admin={admin} adminSitOut={adminSitOut} adminLeave={adminLeave} permissions={permissions} dragging={dragging} drop={drop} beginDrag={beginDrag}/>;
 return <div className={`king-board ${dragging?'king-dragging':''}`}>{courts.map((court,courtIndex)=>{const active=teams.filter(team=>team.status==='current'&&team.court_number===court.court_number).sort((a,b)=>(a.court_side??1)-(b.court_side??1));const ownCourt=active.some(team=>team.id===me?.team_id);const mode=court.team_mode??'rotation';const maxWins=court.team_max_wins;const firstTeam=courtIndex*2+1;const playing=active.reduce((total,team)=>total+team.members.length,0);return <section className="king-court queue-card" key={court.court_number}><header><div className="king-court-title"><h2>{courts.length>1?`COURT ${court.court_number} - Game ${court.game_number}`:'CURRENT GAME - Team 1 vs Team 2'}</h2><div className="court-team-rule">{operator?<><select aria-label={`Court ${court.court_number} format`} value={mode} onChange={event=>void setCourtRules(court.court_number,event.target.value as 'rotation'|'king',maxWins)}><option value="rotation">2 on, 2 off</option><option value="king">King of the Court</option></select>{mode==='king'&&<><span>with</span><select aria-label={`Court ${court.court_number} consecutive games`} value={court.team_max_wins??'unlimited'} onChange={event=>void setCourtRules(court.court_number,'king',event.target.value==='unlimited'?null:Number(event.target.value))}><option value="2">2</option><option value="3">3</option><option value="unlimited">Unlimited</option></select><span>consecutive games MAX</span></>}</>:<span>{mode==='king'?(court.team_max_wins==null?'KING OF THE COURT (No game limit)':`KING OF THE COURT (${court.team_max_wins} CONSECUTIVE GAMES MAX)`):'2 ON, 2 OFF'}</span>}</div></div></header><div className="king-playing-count">{playing} playing</div>{courts.length>1&&operator&&<button className="court-next-button" disabled={busy||active.length!==2||Boolean(me?.restricted)} onClick={()=>nextGame(court.court_number)}>Next game (Court {court.court_number})</button>}<div className="king-current-teams">{active.map((team,index)=>card(team,index*6+1,firstTeam+index,mode==='king'))}{active.length<2&&Array.from({length:2-active.length},(_,index)=>{const side=firstTeam+active.length+index;const targetKey=`empty:current:${court.court_number}:${active.length+index+1}`;return <section data-empty-king-target={targetKey} className="king-team-card queue-team-block empty-team-block" key={`empty-team-${index}`}><div className="team-block-label"><strong>Team {side}</strong>{!operator&&me&&<button className="king-join-button" disabled={busy} onClick={()=>joinEmptyTeam(side)}>Join +</button>}</div>{Array.from({length:6},(__,slot)=>dragging&&drop?.teamId===targetKey&&slot===0?<article className="player-row king-player-row king-drop-placeholder" aria-label="Player drop position" key="drop-placeholder"><span className="position">{(active.length+index)*6+1}</span></article>:<article className="player-row king-player-row king-empty-row" key={slot}><span className="position">{(active.length+index)*6+slot+1}</span><div className="player-name"><strong>Open spot</strong></div></article>)}</section>})}</div></section>})}<section className="king-waitlist queue-card"><header><h2>Waitlist</h2><span>{waiting.length?`${waiting.length} team${waiting.length===1?'':'s'} waiting`:'No one waiting'}</span></header><div className="king-waiting-list">{waiting.length?waiting.map((team,index)=>card(team,13+index*6,courts.length*2+index+1,false)):!hasOpenCourtTeam?<section data-empty-king-target="empty:waiting" className="king-team-card queue-team-block empty-team-block"><div className="team-block-label"><strong>Team {courts.length*2+1}</strong>{!operator&&me&&<button className="king-join-button" disabled={busy} onClick={()=>joinEmptyTeam(courts.length*2+1)}>Join +</button>}</div>{Array.from({length:6},(_,slot)=>dragging&&drop?.teamId==='empty:waiting'&&slot===0?<article className="player-row king-player-row king-drop-placeholder" aria-label="Player drop position" key="drop-placeholder"><span className="position">13</span></article>:<article className="player-row king-player-row king-empty-row" key={slot}><span className="position">{13+slot}</span><div className="player-name"><strong>Open spot</strong></div></article>)}</section>:<p className="empty">Teams will appear here after the active courts are filled.</p>}</div></section></div>
}
