...
function KingBoard({teams,courts,me,admin,host,busy,nextGame,joinTeam,joinEmptyTeam,setCourtRules,movePlayer,adminSitOut,adminLeave,permissions}:{teams:KingTeam[];courts:Court[];me:Player|null;admin:boolean;host:boolean;busy:boolean;nextGame:(court:number)=>void;joinTeam:(team:KingTeam,side:number)=>void;joinEmptyTeam:(side:number)=>void;setCourtRules:(court:number,mode:'rotation'|'king',maxWins:number|null)=>Promise<void>;movePlayer:(playerId:string,targetTeamId:string,index:number)=>Promise<void>;adminSitOut:(player:Player)=>void;adminLeave:(player:Player)=>void;permissions:(player:Player)=>void}){
 ...
 const resolveTarget=(x:number,y:number)=>{const point=constrainQueueDragPoint(x,y);
  ...
  let index=Math.max(0,Math.min(Math.round((point.y-contentTop)/rowHeight),memberCount));const current=dropRef.current;
  // Use a wide, directional dead band around each slot boundary. The target
  // changes only after the pointer clearly crosses the next row, so the
  // placeholder cannot bounce between position 1 and position 2 at the top.
  if(current?.teamId===targetKey&&index!==current.index){
   if(index>current.index&&point.y<contentTop+(current.index+.65)*rowHeight)index=current.index;
   if(index<current.index&&point.y>contentTop+(current.index-.65)*rowHeight)index=current.index;
  }
 ...
 const startAutoScroll=()=>{if(dragRef.current.frame!==null)return;const tick=()=>{const state=dragRef.current;if(!state.active){state.frame=null;return;}const edge=Math.min(144,window.innerHeight*.24);const root=document.scrollingElement??document.documentElement;const canUp=root.scrollTop>1;const canDown=root.scrollTop+window.innerHeight<root.scrollHeight-1;const direction=state.lastY<edge&&canUp?-1:state.lastY>window.innerHeight-edge&&canDown?1:0;if(direction){const distanceToEdge=direction<0?state.lastY:window.innerHeight-state.lastY;const strength=Math.max(.2,1-Math.max(0,distanceToEdge)/edge);window.scrollBy({top:direction*(4+14*strength),behavior:'auto'});if(state.preview)positionPlayerDragPreview(state.preview,state.lastX,state.lastY);resolveTarget(state.lastX,state.lastY)}state.frame=requestAnimationFrame(tick)};dragRef.current.frame=requestAnimationFrame(tick)};
 ...
 useEffect(()=>{if(!operator)return;const preventSelection=()=>{if(document.body.classList.contains('king-drag-holding'))window.getSelection()?.removeAllRanges()};const move=(event:PointerEvent)=>{const state=dragRef.current;state.lastX=event.clientX;state.lastY=event.clientY;if(!state.active){if(state.timer!==null&&Math.hypot(event.clientX-state.startX,event.clientY-state.startY)>18){clearTimeout(state.timer);state.timer=null;state.player=null;state.row=null;state.pointerId=null;document.body.classList.remove('king-drag-holding');document.removeEventListener('touchmove',preventNativeTouchScroll);}return;}event.preventDefault();if(state.preview)positionPlayerDragPreview(state.preview,event.clientX,event.clientY);resolveTarget(event.clientX,event.clientY)};const finish=()=>{const state=dragRef.current;if(state.timer!==null)clearTimeout(state.timer);document.removeEventListener('touchmove',preventNativeTouchScroll);stopAutoScroll();const playerId=state.active?state.player?.id??null:null;const target=state.active?dropRef.current:null;state.preview?.remove();document.body.classList.remove('mobile-admin-dragging','king-drag-holding');dragRef.current={player:null,row:null,pointerId:null,preview:null,timer:null,active:false,lastX:0,lastY:0,startX:0,startY:0,frame:null};dropRef.current=null;setDragging(null);setDrop(null);if(playerId&&target)void movePlayer(playerId,target.teamId,target.index)};document.addEventListener('selectionchange',preventSelection);document.addEventListener('pointermove',move,{passive:false});document.addEventListener('pointerup',finish);document.addEventListener('pointercancel',finish);return()=>{document.removeEventListener('selectionchange',preventSelection);document.removeEventListener('pointermove',move);document.removeEventListener('pointerup',finish);document.removeEventListener('pointercancel',finish);document.removeEventListener('touchmove',preventNativeTouchScroll);document.body.classList.remove('king-drag-holding');stopAutoScroll()}},[operator,teams,busy]);
