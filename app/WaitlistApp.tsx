...
export default function App() {
  ...
  const [permissionPlayer,setPermissionPlayer]=useState<Player|null>(null); const [hostTutorial,setHostTutorial]=useState(false); const [hostTutorialStep,setHostTutorialStep]=useState(0);
  ...
  async function refresh(activeUser?:User|null){
    ...
    const activeUid=(activeUser??user)?.id;const activeHost=Boolean(playerRows.find(item=>item.user_id===activeUid)?.is_host);
  ...
  function showPlayerNotification(notification:GroupNotification){
    if(notification.message.startsWith('HOST_APPOINTED|')){setNotice({title:'You are now a Session Host',message:notification.message.split('|')[1]||'The admin appointed you as a Session Host.',onClose:()=>{setHostTutorialStep(0);setHostTutorial(true)}});return;}
    if(notification.message.startsWith('HOST_REMOVED|')){setHostTutorial(false);setNotice({title:'Host permissions removed',message:notification.message.split('|')[1]||'Your Session Host permissions were removed.'});return;}
...
const hostTutorialSteps=[
 {title:'Advance games',message:'Next Game ends the current game and advances the whole queue. Confirm that the game is actually finished before using it.'},
 {title:'Help players check in',message:'Add Player lets you check in someone who does not have their phone. You can add several people without leaving that screen.'},
 {title:'Manage the live order',message:'Move players or whole groups, create groups, and use Substitute when positions need to be exchanged.'},
 {title:'Handle player needs',message:'Use Sit Out or Remove beside a player, and process walk-in players from Rejoin Requests.'},
 {title:'Correct session actions',message:'Undo and Redo cover up to five recent session actions. History shows what happened during this waitlist.'},
 {title:'Temporary access',message:'You cannot change the facility, waitlist mode, members, restrictions, or reset the waitlist. The admin can remove or transfer your Host role at any time.'},
];
...
function HostTutorial({step,next,back,skip}:{step:number;next:()=>void;back:()=>void;skip:()=>void}){const item=hostTutorialSteps[step];return <><div className="tutorial-scrim"/><section className="tutorial-coach host-tutorial" role="dialog" aria-modal="true"><div className="tutorial-progress"><span>HOST TOUR · {step+1} OF {hostTutorialSteps.length}</span><button onClick={skip}>Skip tutorial</button></div><h2>{item.title}</h2><p>{item.message}</p><div className="tutorial-actions">{step>0&&<button className="neutral" onClick={back}>Back</button>}<button className="next" onClick={next}>{step===hostTutorialSteps.length-1?'Finish':'Next'}</button></div></section></>}
