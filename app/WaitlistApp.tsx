...
function KingBoard({teams,courts,me,admin,host,busy,nextGame,joinTeam,joinEmptyTeam,setCourtRules,movePlayer,adminSitOut,adminLeave,permissions}:{teams:KingTeam[];courts:Court[];me:Player|null;admin:boolean;host:boolean;busy:boolean;nextGame:(court:number)=>void;joinTeam:(team:KingTeam,side:number)=>void;joinEmptyTeam:(side:number)=>void;setCourtRules:(court:number,mode:'rotation'|'king',maxWins:number|null)=>Promise<void>;movePlayer:(playerId:string,targetTeamId:string,index:number)=>Promise<void>;adminSitOut:(player:Player)=>void;adminLeave:(player:Player)=>void;permissions:(player:Player)=>void}){
 ...
 const resolveTarget=(x:number,y:number)=>{const point=constrainQueueDragPoint(x,y);const element=document.elementFromPoint(point.x,point.y) as HTMLElement|null;const card=element?.closest<HTMLElement>('[data-king-team-id]');if(!card){setDrop(null);return;}const team=teams.find(item=>item.id===card.dataset.kingTeamId);if(!team||team.members.filter(member=>member.id!==dragRef.current.player?.id).length>=6){setDrop(null);return;}const rows=[...card.querySelectorAll<HTMLElement>('[data-player-id]')].filter(row=>row.dataset.playerId!==dragRef.current.player?.id);let index=rows.length;for(let i=0;i<rows.length;i++){const rect=rows[i].getBoundingClientRect();if(point.y<rect.top+rect.height/2){index=i;break;}}const next={teamId:team.id,index};if(dropRef.current?.teamId===next.teamId&&dropRef.current.index===next.index)return;dropRef.current=next;setDrop(next)};
 const stopAutoScroll=()=>{if(dragRef.current.frame!==null){cancelAnimationFrame(dragRef.current.frame);dragRef.current.frame=null}};
 const startAutoScroll=()=>{if(dragRef.current.frame!==null)return;const tick=()=>{const state=dragRef.current;if(!state.active){state.frame=null;return;}const edge=Math.min(128,window.innerHeight*.22);const bounds=queueScrollBounds();const direction=state.lastY<edge&&Boolean(bounds&&bounds.top<8)?-1:state.lastY>window.innerHeight-edge&&Boolean(bounds&&bounds.bottom>window.innerHeight-8)?1:0;if(direction){const strength=Math.max(.3,1-Math.min(state.lastY,window.innerHeight-state.lastY)/edge);window.scrollBy(0,direction*(7+17*strength));if(state.preview)positionPlayerDragPreview(state.preview,state.lastX,state.lastY);resolveTarget(state.lastX,state.lastY)}state.frame=requestAnimationFrame(tick)};dragRef.current.frame=requestAnimationFrame(tick)};
 ...
 const beginDrag=(player:Player,event:ReactPointerEvent<HTMLElement>,start:number)=>{if(busy||(event.target as HTMLElement).closest('button,input'))return;event.preventDefault();const row=(event.target as HTMLElement).closest<HTMLElement>('[data-player-id]');if(!row)return;const state=dragRef.current;state.player=player;state.row=row;state.pointerId=event.pointerId;state.lastX=event.clientX;state.lastY=event.clientY;state.startX=event.clientX;state.startY=event.clientY;if(event.pointerType==='mouse')activateDrag(player,start);else{document.addEventListener('touchmove',preventNativeTouchScroll,{passive:false});state.timer=window.setTimeout(()=>activateDrag(player,start),220)}};
 useEffect(()=>{if(!operator)return;const move=(event:PointerEvent)=>{const state=dragRef.current;state.lastX=event.clientX;state.lastY=event.clientY;if(!state.active){if(state.timer!==null&&Math.hypot(event.clientX-state.startX,event.clientY-state.startY)>18){clearTimeout(state.timer);state.timer=null;state.player=null;state.row=null;state.pointerId=null;document.removeEventListener('touchmove',preventNativeTouchScroll);}return;}event.preventDefault();if(state.preview)positionPlayerDragPreview(state.preview,event.clientX,event.clientY);resolveTarget(event.clientX,event.clientY)};const finish=()=>{const state=dragRef.current;if(state.timer!==null)clearTimeout(state.timer);document.removeEventListener('touchmove',preventNativeTouchScroll);stopAutoScroll();const playerId=state.active?state.player?.id??null:null;const target=state.active?dropRef.current:null;state.preview?.remove();document.body.classList.remove('mobile-admin-dragging');dragRef.current={player:null,row:null,pointerId:null,preview:null,timer:null,active:false,lastX:0,lastY:0,startX:0,startY:0,frame:null};dropRef.current=null;setDragging(null);setDrop(null);if(playerId&&target)void movePlayer(playerId,target.teamId,target.index)};document.addEventListener('pointermove',move,{passive:false});document.addEventListener('pointerup',finish);document.addEventListener('pointercancel',finish);return()=>{document.removeEventListener('pointermove',move);document.removeEventListener('pointerup',finish);document.removeEventListener('pointercancel',finish);document.removeEventListener('touchmove',preventNativeTouchScroll);stopAutoScroll()}},[operator,teams,busy]);
