...
export default function App() {
  ...
  const geofenceRemovalInProgress=useRef(false); const expiredRejoinHandled=useRef(false); const lastResumeRefresh=useRef(0); const adminMoveInProgress=useRef(false); const handledNotificationIds=useRef(new Set<string>()); const ownHostStatus=useRef(false); const hostTrackedUserId=useRef<string|null>(null); const hostTransitionHandledAt=useRef(0); const hostAppointmentActive=useRef(false);
  ...
  async function boot(){
    ...
    const channel=supabase.channel('live-waitlist')
      .on('postgres_changes',{event:'*',schema:'public',table:'waitlist_players'},payload=>{
        ...
        if(isOwnPlayer)syncOwnHostStatus(Boolean(changed.is_host),session.user.id);
