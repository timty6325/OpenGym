...
export default function App() {
  ...
  useEffect(()=>{document.body.classList.toggle('team-sub-invite-selecting',Boolean(inviteSubTeamId));return()=>document.body.classList.remove('team-sub-invite-selecting')},[inviteSubTeamId]);
  ...
  function startTeamSubInvite(team:KingTeam){
    setNotice({title:'Invite a Sub',message:'Select one player in the waitlist to invite as a sub. They must accept before becoming a substitute.',confirm:'Continue',actionTone:'success',cancelLabel:'Cancel',cancelTone:'danger',showBack:true,action:async()=>{setInviteSubTeamId(team.id)}});
  }
  function selectTeamSubTarget(player:Player){
    const team=kingTeams.find(item=>item.id===inviteSubTeamId);if(!team)return;
    if(player.team_id===team.id){setNotice({title:'Cannot invite this player',message:'This player is already on your team.'});return;}
  ...
  useEffect(()=>{
    const incoming=teamSubstituteRequests.find(request=>players.find(player=>player.id===request.target_id)?.user_id===user?.id);if(!incoming)return;
...
function TeamSubstituteRoster({team,substitutes,expanded,canManage,busy,toggle,invite}:{team:KingTeam;substitutes:TeamSubstitute[];expanded:boolean;canManage:boolean;busy:boolean;toggle:()=>void;invite:()=>void}){
...
state.preview?.remove();document.body.classList.remove('mobile-admin-dragging','king-drag-holding');dragRef.current={player:null,row:null,pointerId:null,preview:null,timer:null,active:false,lastX:0,lastY:0,startX:0,startY:0,frame:null};dropRef.current=null;setDragging(null);setDrop(null);clearEmptyTarget();if(playerId&&target)void movePlayer(playerId,target.teamId,target.index)};const cancelPointer=(event:PointerEvent)=>{if(event.pointerType==='touch'&&dragRef.current.active)return;finish()};document.addEventListener('selectionchange',preventSelection);document.addEventListener('pointermove',move,{passive:false,capture:true});document.addEventListener('touchmove',touchMove,{passive:false,capture:true});document.addEventListener('pointerup',finish,{capture:true});document.addEventListener('touchend',finish,{capture:true});document.addEventListener('touchcancel',finish,{capture:true});document.addEventListener('pointercancel',cancelPointer,{capture:true});return()=>{document.removeEventListener('selectionchange',preventSelection);document.removeEventListener('pointermove',move,{capture:true});document.removeEventListener('touchmove',touchMove,{capture:true});document.removeEventListener('pointerup',finish,{capture:true});document.removeEventListener('touchend',finish,{capture:true});document.removeEventListener('touchcancel',finish,{capture:true});document.removeEventListener('pointercancel',cancelPointer,{capture:true});document.removeEventListener('touchmove',preventNativeTouchScroll);document.body.classList.remove('king-drag-holding');stopAutoScroll()}},[operator,teams,busy]);
 const card=(team:KingTeam,start:number,side:number,showStreak:boolean)=>{const substitutes=teamSubstitutes.filter(sub=>sub.team_id===team.id);const ownTeam=Boolean(me&&team.members.some(member=>member.id===me.id));const expanded=operator||ownTeam||expandedSubs.has(team.id);const toggle=()=>setExpandedSubs(current=>{const next=new Set(current);if(next.has(team.id))next.delete(team.id);else next.add(team.id);return next});return <div className="team-with-subs" key={team.id}><KingTeamCard team={team} me={me} canJoin={!operator&&!playerJoinLocked&&(!me||!['current','waiting'].includes(me.status)||team.id!==me.team_id)&&team.members.length<6} busy={busy} joinTeam={joinTeam} start={start} side={side} showStreak={showStreak} operator={operator} admin={admin} adminSitOut={adminSitOut} adminLeave={adminLeave} permissions={permissions} dragging={dragging} drop={drop} beginDrag={beginDrag} now={Date.now()} fillIns={fillIns} players={players} allTeams={teams} fillIn={fillIn} unfill={unfill}/><TeamSubstituteRoster team={team} substitutes={substitutes} expanded={expanded} canManage={operator||ownTeam} busy={busy} toggle={toggle} invite={()=>inviteSub(team)}/></div>};
