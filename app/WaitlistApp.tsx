  ...
  async function boot(){
    ...
    const channel=supabase.channel('live-waitlist')
      .on('postgres_changes',{event:'*',schema:'public',table:'waitlist_players'},payload=>{
        ...
        if(isOwnPlayer){
          syncOwnHostStatus(Boolean(changed.is_host),session.user.id);
          setOwnPlayer(changed);
          setPlayers(items=>changed.status==='left'?items.filter(player=>player.id!==changed.id):items.map(player=>player.id===changed.id?changed:player));
        }
