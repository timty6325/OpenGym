...
type Notice = { title:string; message:string; confirm?:string; action?:()=>Promise<void>; onClose?:()=>void; actionTone?:'danger'|'success'; cancelTone?:'neutral'|'danger'|'success'; cancelLabel?:string; cancelAction?:()=>Promise<void>; blocking?:boolean; requestId?:string; showBack?:boolean } | null;
...
function preventNativeTouchScroll(event:TouchEvent){
  // Register this listener at finger-down so mobile browsers do not commit the
  // gesture to page scrolling before the hold finishes. Ordinary swipes stay
  // native; only an activated long-press drag suppresses touch scrolling.
...
export default function App() {
  ...
  const meIsVisibleInQueue=Boolean(meHasActiveStatus&&activeMe&&(!isTeamsMode(config.mode)||kingTeams.some(team=>team.id===activeMe.team_id&&team.members.some(member=>member.id===activeMe.id))));
  ...
  async function refresh(activeUser?:User|null){
    ...
    const {data:fillRows}=await supabase.from('team_fill_ins').select('id,sitter_id,filler_id,destination_team_id,source_team_id,court_number,game_number');setTeamFillIns((fillRows??[]) as TeamFillIn[]);
  ...
  function confirmTeamFillIn(sitter:Player,team:KingTeam){
    if(!me)return;
    setNotice({title:`Fill in for ${sitter.display_name}?`,message:`You will fill in for ${sitter.display_name} on ${team.name} for one game. After that game, you will return to your original team and position.`,confirm:'Fill in',actionTone:'success',cancelLabel:'Cancel',cancelTone:'danger',action:async()=>{await rpc('fill_in_team_spot',{p_sitter_id:sitter.id},false)}});
  }
  ...
  const ownKingTeam=isTeamsMode(config.mode)&&meIsVisibleInQueue&&me?.team_id?kingTeams.find(team=>team.id===me.team_id):null;
  ...
  const canPlayerAdvanceKing=Boolean(ownKingCourt&&!me?.restricted);
  ...
  return <Shell>
    ...
    <main className={`queue-page ${!admin&&me&&!meIsVisibleInQueue?'rejoin-only-queue':''}`}>
      ...
      {isTeamsMode(config.mode)&&<KingBoard teams={kingTeams} courts={courts} me={me} admin={admin} host={host} busy={busy} fillIns={teamFillIns} players={players} fillIn={confirmTeamFillIn} nextGame={confirmTeamNext} joinTeam={confirmJoinKingTeam} joinEmptyTeam={confirmCreateKingTeam} setCourtRules={setTeamCourtRules} movePlayer={moveKingPlayer} adminSitOut={confirmAdminSitOut} adminLeave={confirmAdminLeave} permissions={setPermissionPlayer}/>}
      {isTeamsMode(config.mode)&&!admin&&me&&meIsVisibleInQueue&&['current','waiting','sitout'].includes(me.status)&&<section className="my-actions"><span>Your actions</span><div><div className={`next-game-control ${!canPlayerAdvanceKing&&!(onboarding==='tutorial'&&tutorialStep===0)?'unavailable':''}`}><button className="next next-with-substitute" disabled={busy||(!canPlayerAdvanceKing&&!(onboarding==='tutorial'&&tutorialStep===0))} onClick={()=>{if(ownKingCourt)confirmKingNext(ownKingCourt)}}>Next game{courts.length>1&&ownKingCourt?` (Court ${ownKingCourt})`:''}</button>{!canPlayerAdvanceKing&&!(onboarding==='tutorial'&&tutorialStep===0)&&<small>Only available when your team is in a current game</small>}</div><button className="substitute-action" disabled={busy} onClick={playerSubstituting?cancelSubstitute:startPlayerSubstitute}>{playerSubstituting?'Cancel substitute':'Substitute'}</button><button className="neutral" disabled={busy} onClick={confirmMySitOut}>{me.status==='sitout'?'Unsit':'Sit out'}</button><button className="danger" disabled={busy} onClick={()=>ask('Leave the waitlist?','This removes you from the current game or queue.','Leave',leaveOwnWaitlist)}>Leave</button></div></section>}
...
function WaitlistDisclaimer({mode,language,acknowledge}:{mode:Config['mode'];language:AppLanguage;acknowledge:()=>void}){
 ...
 const details=language==='es'?(rejoin?'Después de salir de la cancha, DEBES volver a ingresar dentro de cinco minutos para conservar tu lugar.':'No necesitas volver a registrarte. Permanecerás en la fila hasta que salgas o estés fuera del rango permitido del centro.'):language==='zh-CN'?(rejoin?'离开球场后，你必须在五分钟内重新加入，才能保留你的位置。':'你无需重新报名。除非你主动离开或超出场馆允许的范围，否则你会一直保留在队列中。'):(rejoin?'After your team rotates off the court, you MUST rejoin within five minutes to keep your place.':'You do not need to sign up again. You stay in line until you leave or move out of range of the facility.');
...
const teamHostTutorialSteps=[
 ...
 {title:'Substitute',message:'Use Substitute to select two players and permanently swap their positions. Their teams stay unchanged unless the swap moves them to another team.',target:'.substitute-tool'},
...
function TutorialDemo({mode,host=false}:{mode:Config['mode'];host?:boolean}){
 ...
 return <section className={`tutorial-mode-demo ${host?'tutorial-host-demo':''}`} aria-hidden="true"><div className="tutorial-demo-scroll"><header className="tutorial-demo-header"><Logo compact/><span className="live-pill"><i/>Demo</span></header><section className="game-heading"><div><span className="kicker">TUTORIAL QUEUE</span><h1>Game 4</h1></div></section>
 {host&&<section className="admin-tools host-tools"><span className="host-actions-title">Host Actions</span><button className="next-game-tool">Next game</button><button className="add-player-tool">Add player</button><button className="rejoin-tool">Rejoin requests</button>{!teams&&<button className="group-create-tool">Create Group</button>}<button className="substitute-tool">Substitute</button></section>}
 {!host&&<section className="my-actions"><span>Your actions</span><div><button className="next next-with-substitute">Next game</button><button className="substitute-action">Substitute</button><button className="neutral">Sit out</button><button className="danger">Leave</button></div></section>}
 {teams?<><section className="queue-card king-court court-section"><header><div className="king-court-title"><h2>CURRENT GAME - Team 1 vs Team 2</h2><div className="court-team-rule"><span className="king-rule-display"><b>KING OF THE COURT</b><small>(2 CONSECUTIVE GAMES MAX)</small></span></div></div></header>{host&&<button className="court-next-button">Next game (Court 1)</button>}<div className="king-current-teams"><section className="king-team-card own-king-team"><div className="team-block-label"><strong>Team 1</strong>{!host&&<button className="king-join-button king-own-team-label">Your team</button>}</div><DemoPlayer position={1} name="You" own/><DemoPlayer position={2} name="Demo teammate"/></section><section className="king-team-card"><div className="team-block-label"><strong>Team 2</strong>{!host&&<button className="king-join-button">Join +</button>}</div><DemoPlayer position={7} name="Jordan"/>{host&&<div className="admin-player-actions"><button className="admin-sitout-button">Sit out</button><button className="admin-leave-button">Remove</button></div>}<DemoPlayer position={8} name="Casey"/></section></div></section><section className="king-waitlist queue-card"><header><h2>Waitlist</h2><span>1 team waiting</span></header><section className="king-team-card"><div className="team-block-label"><strong>Team 3</strong>{!host&&<button className="king-join-button">Join +</button>}</div><DemoPlayer position={13} name="Taylor"/><DemoPlayer position={14} name="Open spot"/></section></section></>:<><section className="court-section"><section className="queue-card"><header><h2>Game 4</h2><span>2 playing</span></header><DemoPlayer position={1} name="You" own/><article className="player-row"><span className="position">2</span><div className="player-name"><strong>Jordan</strong></div>{host&&<div className="admin-player-actions"><button className="admin-sitout-button">Sit out</button><button className="admin-leave-button">Remove</button></div>}</article></section></section><section className="queue-card" data-drop-status="waiting"><header><h2>Waitlist</h2><span>2 waiting</span></header><article className="player-row"><span className="position">13</span><div className="player-name"><strong>Casey</strong></div>{!host&&<button className="group-button tutorial-group-target">Group Up</button>}</article><DemoPlayer position={14} name="Taylor"/></section></>}
...
function HostTutorial({mode,step,next,back,skip}:{mode:Config['mode'];step:number;next:()=>void;back:()=>void;skip:()=>void}){useTutorialInteractionLock();const steps=getHostTutorialSteps(mode);const item=steps[step];const [placement,setPlacement]=useState<'top'|'bottom'>('bottom');useEffect(()=>{document.querySelectorAll('.host-tutorial-focus').forEach(node=>node.classList.remove('host-tutorial-focus'));const target=document.querySelector(`.tutorial-mode-demo ${item.target}`) as HTMLElement|null;if(!target)return;target.classList.add('host-tutorial-focus');target.scrollIntoView({behavior:'auto',block:'center'});const place=target.getBoundingClientRect().top<window.innerHeight/2?'bottom':'top';setPlacement(place);const frame=requestAnimationFrame(()=>requestAnimationFrame(()=>keepTutorialTargetClear(target,'.host-tutorial')));return()=>{cancelAnimationFrame(frame);target.classList.remove('host-tutorial-focus')}},[item.target]);return <><div className="tutorial-scrim"/><section className={`tutorial-coach host-tutorial host-tutorial-${placement}`} role="dialog" aria-modal="true"><div className="tutorial-progress"><span>HOST TOUR · {step+1} OF {steps.length}</span><button onClick={skip}>Skip tutorial</button></div><h2>{item.title}</h2><p>{item.message}</p><div className="tutorial-actions">{step>0&&<button className="neutral" onClick={back}>Back</button>}<button className="next" onClick={next}>{step===steps.length-1?'Finish':'Next'}</button></div></section></>}
function TutorialCoach({mode,step,next,back,skip}:{mode:Config['mode'];step:number;next:()=>void;back:()=>void;skip:()=>void}){useTutorialInteractionLock();const steps=getTutorialSteps(mode);const item=steps[step];const demo='demo'in item&&item.demo;const [placement,setPlacement]=useState<'top'|'bottom'>('bottom');useEffect(()=>{if(demo)return;const selector='target'in item?item.target:null;if(!selector)return;const target=document.querySelector(`.tutorial-mode-demo ${selector}`) as HTMLElement|null;if(!target)return;target.classList.add('tutorial-focus');target.scrollIntoView({behavior:'auto',block:'center'});const place=target.getBoundingClientRect().top<window.innerHeight/2?'bottom':'top';setPlacement(place);const frame=requestAnimationFrame(()=>requestAnimationFrame(()=>keepTutorialTargetClear(target,'.tutorial-coach:not(.host-tutorial)')));return()=>{cancelAnimationFrame(frame);target.classList.remove('tutorial-focus')}},[step,demo,item]);return <><div className="tutorial-scrim"/>{demo&&<section className="tutorial-rejoin-demo" aria-hidden="true"><Logo compact/><span className="kicker">REJOIN WAITLIST</span><h2>Do you want to rejoin?</h2><p>Your position is saved.</p><div className="return-countdown rejoin-countdown"><strong>4:59</strong><span>left to rejoin</span></div><div className="rejoin-actions"><button className="next tutorial-rejoin-highlight" disabled>Rejoin</button><button className="danger" disabled>Leave</button></div></section>}<section className={`tutorial-coach tutorial-coach-${placement} ${demo?'tutorial-coach-rejoin-demo':''}`} role="dialog" aria-modal="true" aria-live="polite"><div className="tutorial-progress"><span>QUICK TOUR · {step+1} OF {steps.length}</span><button onClick={skip}>Skip tutorial</button></div><h2>{item.title}</h2><p>{item.message}</p><div className="tutorial-actions">{step>0&&<button className="neutral" onClick={back}>Back</button>}<button className="next" onClick={next}>{step===steps.length-1?'Finish':'Next'}</button></div></section></>}
...
function KingTeamCard({team,me,canJoin,busy,joinTeam,start,side,showStreak,operator,admin,adminSitOut,adminLeave,permissions,dragging,drop,beginDrag,now,fillIns,players,allTeams,fillIn}:{team:KingTeam;me:Player|null;canJoin:boolean;busy:boolean;joinTeam:(team:KingTeam,side:number)=>void;start:number;side:number;showStreak:boolean;operator:boolean;admin:boolean;adminSitOut:(player:Player)=>void;adminLeave:(player:Player)=>void;permissions:(player:Player)=>void;dragging:string|null;drop:KingDropPlacement|null;beginDrag:(player:Player,event:ReactPointerEvent<HTMLElement>,start:number)=>void;now:number;fillIns:TeamFillIn[];players:Player[];allTeams:KingTeam[];fillIn:(sitter:Player,team:KingTeam)=>void}){
...
function KingBoard({teams,courts,me,admin,host,busy,fillIns,players,fillIn,nextGame,joinTeam,joinEmptyTeam,setCourtRules,movePlayer,adminSitOut,adminLeave,permissions}:{teams:KingTeam[];courts:Court[];me:Player|null;admin:boolean;host:boolean;busy:boolean;fillIns:TeamFillIn[];players:Player[];fillIn:(sitter:Player,team:KingTeam)=>void;nextGame:(court:number)=>void;joinTeam:(team:KingTeam,side:number)=>void;joinEmptyTeam:(side:number)=>void;setCourtRules:(court:number,mode:TeamCourtMode,maxWins:number|null)=>Promise<void>;movePlayer:(playerId:string,targetTeamId:string,index:number)=>Promise<void>;adminSitOut:(player:Player)=>void;adminLeave:(player:Player)=>void;permissions:(player:Player)=>void}){
 ...
 const startAutoScroll=()=>{if(dragRef.current.frame!==null)return;const tick=()=>{const state=dragRef.current;if(!state.active){state.frame=null;return;}const edge=Math.min(176,window.innerHeight*.28);const root=document.scrollingElement??document.documentElement;const scrollTop=Math.max(window.scrollY,root.scrollTop);const scrollHeight=Math.max(root.scrollHeight,document.documentElement.scrollHeight,document.body.scrollHeight);const maxScroll=Math.max(0,scrollHeight-window.innerHeight);const canUp=scrollTop>1;const canDown=scrollTop<maxScroll-1;const direction=state.lastY<edge&&canUp?-1:state.lastY>window.innerHeight-edge&&canDown?1:0;if(direction){const distanceToEdge=direction<0?state.lastY:window.innerHeight-state.lastY;const strength=Math.max(.25,1-Math.max(0,distanceToEdge)/edge);const nextTop=Math.max(0,Math.min(maxScroll,scrollTop+direction*(6+20*strength)));root.scrollTop=nextTop;window.scrollTo({top:nextTop,left:0,behavior:'auto'});if(state.preview)positionPlayerDragPreview(state.preview,state.lastX,state.lastY);resolveTarget(state.lastX,state.lastY)}state.frame=requestAnimationFrame(tick)};dragRef.current.frame=requestAnimationFrame(tick)};
 const activateDrag=(player:Player,start:number)=>{const state=dragRef.current;const row=state.row;if(!row)return;captureDragPickupViewport(row);state.player=player;state.timer=null;state.active=true;document.body.classList.add('king-drag-holding');if(state.pointerId!==null)try{row.setPointerCapture(state.pointerId)}catch{}const preview=createPlayerDragPreview(player,[player],start);preview.classList.add('mobile-admin-drag-preview','king-player-drag-preview');document.body.appendChild(preview);state.preview=preview;document.body.classList.add('mobile-admin-dragging');positionPlayerDragPreview(preview,state.lastX,state.lastY);setDragging(player.id);navigator.vibrate?.(35);resolveTarget(state.lastX,state.lastY);startAutoScroll()};
 const beginDrag=(player:Player,event:ReactPointerEvent<HTMLElement>,start:number)=>{if(busy||(event.target as HTMLElement).closest('button,input'))return;const row=(event.target as HTMLElement).closest<HTMLElement>('[data-player-id]');if(!row)return;const state=dragRef.current;state.player=player;state.row=row;state.pointerId=event.pointerId;state.lastX=event.clientX;state.lastY=event.clientY;state.startX=event.clientX;state.startY=event.clientY;state.scrolling=false;if(event.pointerType==='mouse'){event.preventDefault();window.getSelection()?.removeAllRanges();activateDrag(player,start)}else{document.addEventListener('touchmove',preventNativeTouchScroll,{passive:false});state.timer=window.setTimeout(()=>{window.getSelection()?.removeAllRanges();activateDrag(player,start)},220)}};
 useEffect(()=>{if(!operator)return;const scrollBeforeHold=(event:PointerEvent)=>{if(event.pointerType!=='touch')return;const state=dragRef.current;if(state.active||(!state.scrolling&&state.timer===null))return;const distance=Math.hypot(event.clientX-state.startX,event.clientY-state.startY);if(!state.scrolling&&distance<=12)return;event.preventDefault();if(state.timer!==null){clearTimeout(state.timer);state.timer=null;}const delta=state.lastY-event.clientY;state.scrolling=true;state.lastX=event.clientX;state.lastY=event.clientY;if(delta)window.scrollBy(0,delta)};document.addEventListener('pointermove',scrollBeforeHold,{passive:false,capture:true});return()=>document.removeEventListener('pointermove',scrollBeforeHold,{capture:true})},[operator]);
 useEffect(()=>{if(!operator)return;const moveActiveTouchDrag=(event:PointerEvent)=>{if(event.pointerType!=='touch')return;const state=dragRef.current;if(!state.active)return;event.preventDefault();state.lastX=event.clientX;state.lastY=event.clientY;if(state.preview)positionPlayerDragPreview(state.preview,event.clientX,event.clientY);resolveTarget(event.clientX,event.clientY);startAutoScroll()};document.addEventListener('pointermove',moveActiveTouchDrag,{passive:false,capture:true});return()=>document.removeEventListener('pointermove',moveActiveTouchDrag,{capture:true})},[operator,teams]);
 useEffect(()=>{if(!operator)return;const preventSelection=()=>{if(document.body.classList.contains('king-drag-holding'))window.getSelection()?.removeAllRanges()};const updatePosition=(x:number,y:number,prevent?:()=>void)=>{const state=dragRef.current;state.lastX=x;state.lastY=y;if(!state.active){if(state.timer!==null&&Math.hypot(x-state.startX,y-state.startY)>24){clearTimeout(state.timer);state.timer=null;state.player=null;state.row=null;state.pointerId=null;document.body.classList.remove('king-drag-holding');document.removeEventListener('touchmove',preventNativeTouchScroll);}return;}prevent?.();if(state.preview)positionPlayerDragPreview(state.preview,x,y);resolveTarget(x,y);startAutoScroll()};const move=(event:PointerEvent)=>{if(event.pointerType==='touch')return;updatePosition(event.clientX,event.clientY,()=>event.preventDefault())};const touchMove=(event:TouchEvent)=>{const touch=event.touches[0];if(!touch)return;updatePosition(touch.clientX,touch.clientY,()=>event.preventDefault())};const finish=()=>{const state=dragRef.current;if(state.timer!==null)clearTimeout(state.timer);document.removeEventListener('touchmove',preventNativeTouchScroll);stopAutoScroll();const playerId=state.active?state.player?.id??null:null;
...
<div className="team-block-label"><strong>Team {teamNumberForPlaceholder('empty:waiting',courts.length*2+1)}</strong>{!operator&&me&&<button className="king-join-button" disabled={busy} onClick={()=>joinEmptyTeam(teamNumberForPlaceholder('empty:waiting',courts.length*2+1))}>Join +</button>}</div>{Array.from({length:6},(_,slot)=>dragging&&drop?.teamId==='empty:waiting'&&slot===0?<article className="player-row king-player-row king-drop-placeholder" aria-label="Player drop position" key="drop-placeholder"><span className="position">{waitingPlaceholderStart}</span></article>:<article className="player-row king-player-row king-empty-row" key={slot}><span className="position">{waitingPlaceholderStart+slot}</span><div className="player-name"><strong>Open spot</strong></div></article>)}</section>:waiting.length===0?<p className="empty">Teams will appear here after the active courts are filled.</p>:null}</div></section></div>
}
function Modal({notice,close,busy}:{notice:Exclude<Notice,null>;close:()=>void;busy:boolean}){const dismiss=async()=>{close();await notice.cancelAction?.();notice.onClose?.()};const emphasized=notice.message.match(/^(.*?) \*\*(.+)\*\*$/);return <div className="modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget&&!notice.blocking&&!notice.onClose)void dismiss()}}><section className={`modal${notice.showBack?' has-modal-back':''}`} role="dialog" aria-modal="true">{notice.showBack&&<button className="modal-back-button" aria-label="Back" disabled={busy} onClick={close}>{'\u2190'}</button>}<span className="modal-mark">OG</span><h2>{notice.title}</h2>{emphasized?<p>{emphasized[1]}<strong className="modal-reminder">**{emphasized[2]}**</strong></p>:<p>{notice.message}</p>}<div className="modal-actions">{notice.action&&<button className={notice.actionTone==='success'?'next':'danger'} disabled={busy} onClick={async()=>{close();await notice.action?.();notice.onClose?.()}}>{notice.confirm}</button>}<button className={notice.cancelTone==='danger'?'danger':notice.cancelTone==='success'?'next':'neutral'} disabled={busy} onClick={()=>void dismiss()}>{notice.cancelLabel??(notice.action?'Cancel':'OK')}</button></div></section></div>}
