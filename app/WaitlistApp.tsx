...
<div className="team-block-label"><strong>Team {courts.length*2+1}</strong>{!operator&&me&&<button className="king-join-button" disabled={busy} onClick={()=>joinEmptyTeam(courts.length*2+1)}>Join +</button>}</div>{Array.from({length:6},(_,slot)=>dragging&&drop?.teamId==='empty:waiting'&&slot===0?<article className="player-row king-player-row king-drop-placeholder" aria-label="Player drop position" key="drop-placeholder"><span className="position">13</span></article>:<article className="player-row king-player-row king-empty-row" key={slot}><span className="position">{13+slot}</span><div className="player-name"><strong>Open spot</strong></div></article>)}</section>:<p className="empty">Teams will appear here after the active courts are filled.</p>}</div></section></div>
}
